...
```text
templates/_bundled/glitter-gallery
```

활성 템플릿 경로는 직접 수정 대상이 아니며, 아래 명령으로 생성되는 출력 디렉토리입니다.

```text
templates/glitter-gallery
```
...
```bash
cd templates/_bundled/glitter-gallery
npm run build
```
...
- `dist/*` must be readable by the web server after build and after `template:update`.
- Use permission modes that keep assets readable in shared NAS environments, such as `755` or `775`.
- If build output ownership changes during deployment, fix ownership and mode before serving `dist/css/components.css` and `dist/js/components.iife.js`.
